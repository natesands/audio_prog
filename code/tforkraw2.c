...
/* also generates text file with values */
/* based on tfork2.c */
...
/* return 0 for big-endian machine, 1 for 
...
int main(int argc, char** argv)
{
  ...
  angleincr = twopi * freq / nsamps; /* rad per sample */
  step = dur / nsamps; /* seconds per sample */
  ...
  if(samptype==RAWSAMP_SHORT) {
    for(i=0;i<nsamps;i++){
      samp = amp * sin(angleincr*i); /* a*sin(theta) */
      samp *= start;
      start *=fac;
